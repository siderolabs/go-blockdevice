...
//
// A device which was only just created is held open for as long as whatever watches block devices
// takes to probe it, so a caller removing a device it has just made should expect unix.EBUSY and
// retry: this is the race 'dmsetup remove --retry' exists for.
